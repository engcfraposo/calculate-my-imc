...
export const storeImc = async (data: any) => {
  try {
    let historicData: any = await AsyncStorage.getItem('@historicImc');
    const key: any = moment().format('YYYY-MM-DD');
    ...
    const newHistoricData: Object = {[key]: data};
...
export const getHistoric = async () => {
  const historicData: any =  await AsyncStorage.getItem('@historicImc');

  return JSON.parse(historicData);
};

export const getClassification = (val: number) => {
...
export const calculate = (weight: number, height: number) => {
